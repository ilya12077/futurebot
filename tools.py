...
def count_duplicate_messages(user_id: str, message: str = None, file_unique_id: str = None) -> int:
    count = 0
    with open(f'history.txt', 'r', encoding='utf-8') as f:
...
def clear_history():
    with open(f'{path}history.txt', 'r', encoding='utf-8') as f:
        a = f.readlines()
        for index, record in enumerate(a):
            if int(time.time()) - int(record.split()[0]) > spam_timeout:
                a[index] = ''
    with open(f'{path}history.txt', 'w', encoding='utf-8') as f:
        f.write(''.join(a))


def append_history(user_id: int | str, r: str, date=time.time):
    clear_history()
    with open(f'{path}history.txt', 'a', encoding='utf-8') as f:
        try:
